...
- Arm-specific preset names gate only the TQ render board: `Left Arm` / `LEFT_ARM` presets can use `F`/`FH` but only search TQ images in `TQ-L`; `Right Arm` / `RIGHT_ARM` presets can use `F`/`FH` but only search TQ images in `TQ-R`.
